...
class CompleteBinaryTree
{
    ...
    public:
        ...
        void CreateBinTree(T *stu, unsigned n) {CreateBinTree(stu, root, n);}               //创建完全二叉树,n为创建结点数
...
void CompleteBinaryTree<T>::CreateBinTree(T *stu, BinTreeNode<T> *&subTree, unsigned n)
{
    ...
    subTree = new BinTreeNode<T>(stu[0]); if (subTree == NULL) {cerr << "存储分配错误！" << endl; exit(1);}    //新建根结点
    ...
    while (cnt < size)
    {
        ...
        newNode = new BinTreeNode<T>(stu[cnt - 1]); if (newNode == NULL) {cerr << "存储分配错误！" << endl; exit(1);}
...
bool CompleteBinaryTree<T>::save(const char *path) const
{
    ...
    while (!Q.isEmpty())                                                   //队列不为空
    {
        Q.DeQueue(current); outfile.write((char *)&current->data, sizeof(current->data));             //退出一个结点，写入文件
        if (current->child[0] != NULL) Q.EnQueue(current->child[0]);      //左子女进队
        if (current->child[1] != NULL) Q.EnQueue(current->child[1]);      //右子女进队
...
bool CompleteBinaryTree<T>::load(const char *path)
{
    ...
    while (infile.read((char *) &item, sizeof(item)))
    {
        ...
        Q.EnQueue(newNode);                                                 //左、右子女进队
        temp->child[size % 2] = newNode;                                    //新结点入树
